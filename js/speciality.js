var speciality_table;

function filterGlobal() {
    $('#speciality_table').DataTable().search(
        $('#global_filter').val(),
    ).draw();
}

function ListSpeciality(){
    speciality_table = $("#speciality_table").DataTable({
       "ordering":false,
       "paging": false,
       ...
       "processing": true,
       "ajax":{
           "url":"../controller/speciality/list_speciality.php",
           type:'POST'
       },
       "order":[[1, 'asc']],
       "columns":[
   ...
   document.getElementById("speciality_table_filter").style.display="none";
    ...
    speciality_table.on( 'draw.dt', function () {
    var PageInfo = $('#speciality_table').DataTable().page.info();
    speciality_table.column(0, { page: 'current' }).nodes().each( function (cell, i) {
